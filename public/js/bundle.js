(() => {
  ...
  // public/js/createaccount.js
  var createaccount = async (customerNumber, firstName, lastName, company, email, password, passwordConfirm) => {
    try {
      const res = await axios_default({
        method: "POST",
        url: `/api/v1/users/`,
  ...
  var updateSettings = async (data, type) => {
    try {
      const url = type === "password" ? "/api/v1/users/updateMyPassword" : "/api/v1/users/updateAccount";
      const res = await axios_default({
        method: "PATCH",
        url,
        data
  ...
  require_noConflict();
  var _global2 = _interopRequireDefault(require_global3());
  function _interopRequireDefault(obj) {
    return obj && obj.__esModule ? obj : { "default": obj };
  }
  if (_global2["default"]._babelPolyfill && typeof console !== "undefined" && console.warn) {
    console.warn("@babel/polyfill is loaded more than once on this page. This is probably not desirable/intended and may have consequences if different versions of the polyfills are applied sequentially. If you do need to load the polyfill more than once, use @babel/polyfill/noConflict instead to bypass the warning.");
  }
  _global2["default"]._babelPolyfill = true;
  ...
  var resetpassword = async (passwordResetToken, password, passwordConfirm) => {
    try {
      const res = await axios_default({
        method: "PATCH",
  ...
  var newMessage = async (date, time, name, email, phone, requestType, service, message, contactMethod) => {
    try {
      const res = await axios_default({
        method: "POST",
        url: `/api/v1/messages/`,
  ...
  var emailReg = async (date, time, email) => {
    try {
      const res = await axios_default({
        method: "POST",
  ...
  if (ogfile) {
    ...
    const calculatedSize = calcWidth.toFixed(2) + " in. wide x " + calcHeight.toFixed(2) + " in. tall";
    document.getElementById("original-size").setAttribute("value", calculatedSize);
  }
  var filter2 = document.getElementById("filter");
  var box1 = document.getElementById("brandsCol");
  var box2 = document.getElementById("categoryCol");
  var button = document.getElementById("resultsBtn");
  var buttonDiv = document.getElementById("submitBtn");
  if (filter2) {
    filter2.addEventListener("change", function handleChange(event) {
  ...
  var productFilter = document.getElementById("product-filter-form");
  if (productFilter) {
    productFilter.addEventListener("submit", (e) => {
      e.preventDefault();
      const filter3 = document.getElementById("filter").value;
      const brands = document.getElementById("brands").value;
      const category = document.getElementById("category").value;
      if (filter3 === "brand" && brands === "") {
        showAlert(
          "error",
      ...
      } else if (filter3 === "category" && category === "Tank Tops") {
        window.location.href = "/tanks";
      } else if (filter3 === "brand") {
        window.location.href = `/filter?${filter3}=${brands}#product-results`;
      } else {
        window.location.href = `/filter?${filter3}=${category}#product-results`;
      }
    });
  }
  ...
  var quoteRequest = document.getElementById("quoteForm");
  if (quoteRequest) {
  ...
  if (profileUpdateForm) {
    profileUpdateForm.addEventListener("submit", (e) => {
      ...
      const data = {
        ...
        lastName,
        company,
        address: { street1: street, city, state, zip },
        phone,
        email
      };
  ...
  var billingUpdateForm = document.getElementById("billing-edit");
  ...
  var shippingUpdateForm = document.getElementById("shipping-edit");
